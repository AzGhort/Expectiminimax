...
import java.util.*;
import conquest.bot.playground.expectiminimax.*;
...
class TGenerator implements Generator<TState, Integer> {
	...
	public List<Possibility<TState>> possibleResults(TState state, Integer action) {
		return Arrays.asList(new Possibility<TState>(1.0, state.result(action)));
	}
}
...
public class TicTacToe implements Game<TState, Integer> {
    ...
    public static void main(String[] args) {
		...
		Strategy<TState, Integer> emm = new Expectiminimax<>(game, new TGenerator(), new TEvaluator(), 4); 
